 ...
 * \brief A wrapper class for a WebGPU buffer.
 * 
 * \details This class provides a simple interface for creating, updating, and destroying a WebGPU buffer.
 * It manages the underlying WGPUBuffer and ensures proper alignment of the buffer size.
 */
class WgpuBuffer {
public:
	 ...
	 *
	 * \details Returns the aligned size of the buffer, which is the size rounded up to the nearest multiple of 256 bytes.
	 *
...
private:
	...
	size_t m_alignedSize; ///< The aligned size of the buffer (rounded up to the nearest multiple of 256 bytes)
};
