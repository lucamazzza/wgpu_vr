...
\noindent Ciò permette di forzare l'astrazione e di risalire all'istanza Vulkan nativa, restituendola all'API C++
...
Il problema più critico si presenta quando si deve gestire la \textit{\gls{Swapchain}}. \texttt{OpenXR} richiede che i
fotogrammi vengano renderizzati direttamente sulle sue \texttt{VkImage}.
...
Sapendo per costruzione che il primo campo della struttura \texttt{Texture} nel backend Vulkan di WebGPU corrisponde
all'handle nativo \texttt{ash::vk::Image}, è possibile bypassare la protezione del compilatore castando
direttamente l'indirizzo di memoria della struttura ad un puntatore Vulkan.
