...
questo problema, il \textit{Khronos Group} ha rilasciato \texttt{OpenXR}, l'attuale standard industriale open-source
per lo spatial computing, adottato universalmente da tutti i principali produttori, inclusa Valve stessa, che ha
attivamente incoraggiato l'abbandono di \texttt{OpenVR}.
...
\begin{itemize}
    ...
    \item \textbf{Gestione Spazi/Tracking}: \texttt{OpenVR} restituisce le matrici di posizionamento (\textit{Poses})
        basate su un sistema di coordinate cablato. \texttt{OpenXR} invece introduce il concetto id \texttt{XrSpace},
        dove ogni elemento, che si tratti del visore, del controller o della zona di gioco, è codificato come "Spazio".
